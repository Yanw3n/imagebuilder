...
while read -r package _; do
  ...
  if test "$PROFILE" = rescue; then
    case "$package" in
      dae|daed|luci-app-daede|adguardhome|tailscale|tailscaled|python3|python3-*|vlmcsd|luci-app-vlmcsd|nvme-cli|smartmontools|ethtool|ip-full|iperf3|tcpdump|usbutils|pciutils)
