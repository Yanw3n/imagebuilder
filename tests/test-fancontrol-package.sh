...
test "$(grep -c 'BuildPackage' "$luci_mk")" = 1 || fail 'LuCI package has a missing or duplicate BuildPackage discovery signature'
! grep -Fq '$(eval $(call BuildPackage' "$luci_mk" || fail 'luci.mk package has an explicit duplicate BuildPackage registration'
